...
d = 2*(R-r0)./(J - 1); 
%Mean cosine (varied between 0 and 0.9)
mean_cosine = 0; 
%Concentration parameter (see Eqn 7 in the manuscript)
fv = @(kap) (mean_cosine - besseli(1,kap)./besseli(0,kap));
kappa = round(fzero(fv,0.5),4);
...
theta(k,a) = vmrand(theta(k,a - 1), kappa);
